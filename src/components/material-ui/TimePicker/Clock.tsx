...
class Clock extends React.PureComponent<Props, State> {
  ...
  componentWillReceiveProps({ value, mode }: Props) {
    ...
    return (
      <div className={classes.root} {...other}>
        ...
        >
          ...
          {mode === '12h' && getNumbers(12, { size }).map((digit: any, i: number) => (
            <span
              key={digit.display}
          ...
          {mode === '24h' && getNumbers(12, { size }).map((digit: any, i: number) => (
            <span
              key={digit.display}
              className={classNames(classes.number, { selected: value === digit.display })}
              style={{
          ...
          {mode === '24h' && getNumbers(12, { size: size - 64, start: 13 }).map((digit: any, i: number) => (
            <span
              key={digit.display}
          ...
          {mode === 'minutes' && getNumbers(12, { size, start: 5, step: 5 }).map((digit: any, i: number) => (
            <span
              key={digit.display}
...
function getNumbers(count: number, { size, start = 1, step = 1 }: { size: number, start?: number, step?: number }) {
  return Array.apply(null, Array(count)).map((_: any, i: number) => ({
...
function getPointerAngle(value: number, mode: string): number {
  switch (mode) {
    case '12h':
...
function getPointerValue(x: number, y: number, mode: string) {
