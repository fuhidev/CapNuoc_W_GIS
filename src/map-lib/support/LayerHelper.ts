...
export default class LayerHelper {
  ...
  public static getSuCoRenderer() {
    ...
    function renderUniqueValue(trangThai: SuCo.TrangThai) {
      const value = trangThai;
      return {
        value,
    ...
    [SuCo.TrangThai.MoiTiepNhan, SuCo.TrangThai.HoanThanh]
      .forEach(linhVuc =>
        uniqueValueInfos.push(renderUniqueValue(linhVuc)));
    let renderer = new UniqueValueRenderer({
      field: 'TrangThai',
      uniqueValueInfos,
      defaultSymbol: new PictureMarkerSymbol({
        width: 20, height: 20,
        url: `${baseUrl}/marker.png`
      })
    });
    return renderer;
  }
